...
import shutil
import provider
from config.setting import cal_loss,optimizer_set
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='RiDBNet_seg', help='model name')
...
def main(args):
    ...
    TRAIN_DATASET = PartNormalDataset(root=root, npoints=args.npoint, split='trainval', use_normals=args.normal,z=False,SO3=False)
    ...
    TEST_DATASET = PartNormalDataset(root=root, npoints=args.npoint, split='test', use_normals=args.normal,z=False,SO3=False)
    ...
    shutil.copy('./train_partseg_shapenet.py', str(exp_dir))
    shutil.copy('./config/setting.py', str(exp_dir))


    classifier = MODEL.get_model(num_part, use_normals=args.normal)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            seg_pred, feature = classifier(points, to_categorical(label, num_classes))
            seg_pred = seg_pred.contiguous().view(-1, num_part)
            target = target.view(-1, 1)[:, 0]
            pred_choice = seg_pred.data.max(1)[1]
            correct = pred_choice.eq(target.data).cpu().sum()
            mean_correct.append(correct.item() / (args.batch_size * args.npoint))
            loss = criterion[0](seg_pred, target)
            loss.backward()
            optimizer.step()
