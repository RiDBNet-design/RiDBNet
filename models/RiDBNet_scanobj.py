...
class get_model(nn.Module):
    def __init__(self, num_category=40,npoint_n=2,use_normals=True,if_train=False):
        ...
        self.sc0 = RiDBNetSetAbstraction(in_c=0,  out_c=6,  embed_channel=0,   out_channel=64,  gp=2, n_sample=512*npoint_n, k=24,  radi=None, first=True)
        self.sc1 = RiDBNetSetAbstraction(in_c=6,  out_c=32, embed_channel=64,  out_channel=128, gp=4, n_sample=256*npoint_n, k=24, radi=0.2)
        self.sc2 = RiDBNetSetAbstraction(in_c=32, out_c=64, embed_channel=128, out_channel=256, gp=4, n_sample=128*npoint_n, k=24, radi=0.4)
        self.sc3 = RiDBNetSetAbstraction(in_c=64, out_c=128, embed_channel=256, out_channel=512, gp=8, n_sample=64*npoint_n, k=24, radi=0.8)
    ...
    def forward(self, x):
        ...
        fpsx_idx = furthest_point_sample(x_global,512*self.npoint_n).long()
        x_global = index_points(x_global, fpsx_idx).transpose(2,1)

        x_global,_,group_points_max,x,normal,new_points = self.sc0(x_global.transpose(2,1),x_global,formal_x,normal.transpose(2,1),None)
